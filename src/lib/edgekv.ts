...
export default class EdgeKV {
	...
	constructor(namespace = "default", group = "default") {
		if (typeof namespace === "object") {
			this.#namespace = ('namespace' in namespace) ? namespace['namespace'] : "default";
			this.#group = ('group' in namespace) ? namespace['group'] : "default";
			this.#edgekv_uri = ('edgekv_uri' in namespace) ? namespace['edgekv_uri'] : "https://edgekv.akamai-edge-svcs.net";
			this.#token_override = ('token_override' in namespace) ? namespace['token_override'] : null;
		} else {
	...
	validate({ namespace, group, item } : Partial<{namespace, group, item}> ) {
		if (namespace !== undefined && !/^[A-Za-z0-9_-]{1,32}$/.test(namespace)) {
			throw "Namespace is not valid. Must be 32 characters or less, consisting of A-Z a-z 0-9 _ or -";
		}
		if (group !== undefined && !/^[A-Za-z0-9_-]{1,128}$/.test(group)) {
			throw "Group is not valid. Must be 128 characters or less, consisting of A-Z a-z 0-9 _ or -";
		}
		if (item !== undefined && !/^[A-Za-z0-9_-]{1,512}$/.test(item)) {
	...
	putRequest( item:string, value:any, timeout:number = 1000 ) : Promise<HttpResponse> {
		this.validate({ item });
		let uri = this.#edgekv_uri + "/api/v1/namespaces/" + this.#namespace + "/groups/" + this.#group + "/items/" + item;
		return httpRequest(uri, this.addTimeout({
			method: "PUT",
			body: typeof value === "object" ? JSON.stringify(value) : value,
			headers: { "X-Akamai-EdgeDB-Auth": [this.getNamespaceToken(this.#namespace)] }
		}, timeout));
	}
	 ...
	 */
	async putText(item:string, value:any, timeout:number = 1000) : Promise<HttpResponse> {
		return this.requestHandlerTemplate(
			() => this.putRequest( item, value ),
	 ...
	 */
	putTextNoWait(item:string, value:any) {
		try {
			this.putRequest(item, value);
		} catch (error) {
	 ...
	 * 		an object describing the non-200 response from the EdgeKV: {failed, status, body}
	 */
	async putJson(item:string, value:any, timeout = 1000) {
		return this.requestHandlerTemplate(
			() => this.putRequest(item, JSON.stringify(value), timeout),
			(response) => response.text(),
			(response) => this.streamText(response.body),
			"PUT",
			null
	 ...
	 * 		an object describing the error: {failed, status, body}
	 */
	putJsonNoWait(item:string, value:any) {
		try {
			this.putRequest(item, JSON.stringify(value));
		} catch (error) {
			this.throwError("PUT FAILED", 0, error.toString());
		}
	}

	getRequest(item:string, timeout = 1000) {
		this.validate({ item });
		let uri = this.#edgekv_uri + "/api/v1/namespaces/" + this.#namespace + "/groups/" + this.#group + "/items/" + item;
		return httpRequest(uri, this.addTimeout({
			method: "GET",
	 ...
	 */
	async getText(item:string, default_value = '', timeout = 1000) {
		return this.requestHandlerTemplate(
			() => this.getRequest(item, timeout),
			(response) => response.text(),
			(response) => this.streamText(response.body),
			"GET TEXT",
			default_value
	 ...
	 * 		an object describing the non-200 and non-404 response from the EdgeKV: {failed, status, body}
	 */
	async getJson(item:string, default_value = {}, timeout = 1000) {
		return this.requestHandlerTemplate(
			() => this.getRequest(item, timeout),
	...
	deleteRequest(item, timeout = 1000) {
		this.validate({ item });
		let uri = this.#edgekv_uri + "/api/v1/namespaces/" + this.#namespace + "/groups/" + this.#group + "/items/" + item;
		return httpRequest(uri, this.addTimeout({
			method: "DELETE",
	 ...
	 * 		an object describing the non-200 response from the EdgeKV: {failed, status, body}
	 */
	async delete(item:string, timeout = 1000) {
		return this.requestHandlerTemplate(
			() => this.deleteRequest(item, timeout),
			(response) => response.text(),
			(response) => this.streamText(response.body),
			"DELETE",
			null
